...
export class VenuePageComponent implements OnInit {
    ...
    this.venueService.findVenueById(venueId).subscribe({
      next: (response: VenueOverviewDto) => {
        ...
        for (let i = 0; i < this.VenueTypes.length; i ++) {
          if (this.VenueTypes[i].name == this.VenueTypeMap.get(this.venue.type)) {
        ...
        this.editVenueForm = new FormGroup({
          ...
          description: new FormControl(this.venue?.description, Validators.required),
          type: new FormControl(this.VenueTypes[this.initialVenueTypeIndex], Validators.required),
        })
      },
      error: (error) => {},
    });
  ...
  submitChanges() {
    ...
    let updateVenueRequest = {
      ...
      venueType: this.VenueTypeConversionMap.get(this.editVenueForm.value.type.value) ?? 0
    }
